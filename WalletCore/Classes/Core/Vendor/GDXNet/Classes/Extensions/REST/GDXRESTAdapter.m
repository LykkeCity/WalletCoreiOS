...
- (NSString *)send:(GDXNetPacket *)packet userInfo:(NSDictionary *)userInfo {
    ...
    void (^accept)(NSURLSessionDataTask *task, id responseObject, NSError *error) = ^(NSURLSessionDataTask *task, id responseObject, NSError *error) {
        ...
        if(responseObject == nil && error == nil) {
            error = [[NSError alloc] initWithDomain:@"" code:0 userInfo:@{NSLocalizedDescriptionKey: @"We are experiencing technical problems. Please try again later.\nServer returned empty data.",
        ...
        if (ctx.isCancelled) { // cancelled request
            ...
            NSURL *ttt=task.currentRequest.URL;
 //           NSAssert(0, @"Potentially invalid network operation.");
    ...
    constructingBodyWithBlock:pack.bodyConstructionBlock
        ...
        case GDXRESTPacketTypePUT: {
            
               [manager PUT:pack.urlRelative
                   parameters:pack.params
                      success:^(NSURLSessionDataTask *task, id response) {
                          accept(task, response, nil);
                      }
...
//
            ...
            NSString *str = pack.urlRelative;
            
            [manager DELETE:pack.urlRelative parameters:nil success:^(NSURLSessionDataTask *task, id response) {
                accept(task, response, nil);
            } failure:^(NSURLSessionDataTask *task, NSError *error) {
                accept(task, nil, error);
            }];
...
//                [manager DELETE:pack.urlRelative parameters:nil success:^(NSURLSessionDataTask *task, id response) {
//                    accept(task, response, nil);
//                } failure:failure:^(NSURLSessionDataTask *task, NSError *error) {
//                    accept(task, nil, error);
//                }];
...
//            if (pack.bodyConstructionBlock) {
//                [manager DELETE:pack.urlRelative
//                   parameters:pack.params
//    constructingBodyWithBlock:pack.bodyConstructionBlock
//                     progress:nil
//                      success:^(NSURLSessionDataTask *task, id response) {
