...
open class SignUpRegistrationViewModel {
    ...
    public lazy var isValid: Observable<Bool> = {
        return Observable.combineLatest( self.password.asObservable(), self.reenterPassword.asObservable(), resultSelector: {(password, reenterpass) -> Bool in
    ...
    public lazy var isValidHint: Observable<Bool> = {
        return Observable.combineLatest( self.hint.asObservable(), self.fake.asObservable(), resultSelector: {(hint, _) -> Bool in
